package com.zzsong.monitor.edge.infrastructure.utils.timewheel;
...
public class HashedWheelTimer implements Timer {
  ...
  private final BlockingQueue<HashedWheelTimerFuture> waitingTasks = new LinkedBlockingQueue<>();
  private final BlockingQueue<HashedWheelTimerFuture> canceledTasks = new LinkedBlockingQueue<>();
  ...
  public HashedWheelTimer(long tickDuration, int ticksPerWheel, int processThreadNum) {
    ...
    if (processThreadNum <= 0) {
      ...
      ThreadFactory threadFactory = BasicThreadFactory.builder().namingPattern("HashedWheelTimer-Executor-%d").build();
      ...
      taskProcessPool = new ThreadPoolExecutor(core, 4 * core,
        60, TimeUnit.SECONDS,
        queue, threadFactory, (r, p) -> {
        log.warn("[TimeWheelPool] ThreadPool[{}] overload, the task[{}] will run by caller thread!", p, r);
        if (!p.isShutdown()) {
          r.run();
        }
      });
  ...
  @Override
  public TimerFuture schedule(@Nonnull TimerTask task, long delay, @Nonnull TimeUnit unit) {
  ...
  private void runTask(@Nonnull HashedWheelTimerFuture timerFuture) {
